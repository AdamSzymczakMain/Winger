...
import { ArrowLeft, Copy, RefreshCw, Heart, AlertCircle } from 'lucide-react-native';
    ...
    return (
      <LinearGradient colors={['#0f172a', '#2e1065', '#0f172a']} style={styles.gradient}>
        ...
        <ScrollView contentContainerStyle={{ padding: 24 }}>
          {error && (
            <View style={[styles.card, { backgroundColor: 'rgba(239, 68, 68, 0.1)', borderColor: 'rgba(239, 68, 68, 0.3)', marginBottom: 16 }]}>
            ...
            <View style={{ marginBottom: 20 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <TouchableOpacity onPress={() => setImageUri(null)}>
                  <Text style={{ color: '#ef4444', fontSize: 14 }}>🗑️ {t.conv_remove || 'Usuń'}</Text>
                </TouchableOpacity>
              </View>
              <View style={[styles.card, { padding: 0, overflow: 'hidden', marginBottom: 0 }]}>
                <Image 
                  source={{ uri: imageUri }} 
                  style={{ width: '100%', height: 250, resizeMode: 'cover' }}
                />
              </View>
              <View style={[styles.card, { backgroundColor: 'rgba(168, 85, 247, 0.1)', borderColor: 'rgba(168, 85, 247, 0.3)', marginTop: 12 }]}>
                <Text style={{ color: '#a855f7', fontSize: 13 }}>
                  💡 AI przeanalizuje to zdjęcie i zaproponuje kreatywne rozpoczęcie rozmowy!
                </Text>
              </View>
            </View>
          )}
          ...
          <Text style={{ color: 'white', fontSize: 16, fontWeight: '500', marginBottom: 12 }}>
            {imageUri 
              ? `${t.conv_context || 'Dodatkowy kontekst'} ${t.conv_optional || '(opcjonalnie)'}`
              : t.conv_placeholder
            }
          </Text>
          <TextInput 
            ...
            placeholder={imageUri 
              ? (t.conv_context_placeholder || 'Możesz dodać dodatkowy kontekst...')
          ...
          {!imageUri && (
            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 24, marginBottom: 24 }}>
              ...
              <Text style={{ color: '#64748b', marginHorizontal: 16, fontSize: 14 }}>
                {t.conv_or || 'lub'}
              </Text>
              <View style={{ flex: 1, height: 1, backgroundColor: 'rgba(255,255,255,0.1)' }} />
            </View>
          ...
          {/* TONACJE */}
          <Text style={{ color: 'white', marginTop: imageUri ? 24 : 0, marginBottom: 12, fontSize: 16 }}>{t.conv_tonalities}</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{gap: 12}}>
            {[
              ...
              <TouchableOpacity key={tone.id} onPress={() => setTonality(tone.id)} style={{ paddingVertical: 10, paddingHorizontal: 20, borderRadius: 20, backgroundColor: tonality === tone.id ? '#db2777' : 'rgba(30, 41, 59, 0.8)' }}>
          ...
          {!imageUri && (
            <View style={{ marginTop: 24 }}>
              <Text style={{ color: 'white', marginBottom: 8 }}>{t.conv_pers} <Text style={{ color: '#64748b', fontSize: 12 }}>(AI Vision)</Text></Text>
              <TouchableOpacity onPress={handlePickImage} style={{ borderWidth: 2, borderColor: 'rgba(234, 179, 8, 0.5)', borderStyle: 'dashed', padding: 16, borderRadius: 12, alignItems: 'center', backgroundColor: 'rgba(15, 23, 42, 0.5)' }}>
                <Text style={{ fontSize: 32, marginBottom: 8 }}>📸</Text>
                <Text style={{ color: '#eab308', fontWeight: '600', fontSize: 16 }}>{t.conv_attach}</Text>
                <Text style={{ color: '#64748b', fontSize: 12, marginTop: 4 }}>
                  {t.conv_image_hint || 'Dodaj zdjęcie osoby, miejsca lub aktywności'}
                </Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          {generatedText && (
            <View style={[styles.card, { marginTop: 24, backgroundColor: 'rgba(34, 197, 94, 0.1)', borderColor: 'rgba(34, 197, 94, 0.3)' }]}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <Text style={{ color: '#22c55e', fontWeight: '600', fontSize: 16 }}>✨ Wygenerowana rozmowa</Text>
                <TouchableOpacity onPress={copyText}>
                  <Copy color="#22c55e" size={18} />
                </TouchableOpacity>
              </View>
              <Text style={{ color: '#e2e8f0', fontSize: 15, lineHeight: 22 }}>{generatedText}</Text>
            </View>
          )}
          ...
          <TouchableOpacity 
            ...
            style={[styles.btnGreen, { opacity: ((!text.trim() && !imageUri) || isGenerating) ? 0.5 : 1 }]}
          >
            {isGenerating ? (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                <ActivityIndicator color="white" />
                <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 16 }}>
                  {imageUri ? (t.conv_analyzing_image || 'Analizuję zdjęcie...') : (t.conv_generating || 'Generuję...')}
                </Text>
              </View>
            ) : (
              <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 16 }}>{t.conv_gen} 🪄✨</Text>
            )}
          </TouchableOpacity>
        </ScrollView>
      </LinearGradient>
...
export const AwkwardSituationScreen = ({ t, onNavigate }) => {
  ...
  const urgencyLevels = [
    ...
    { id: 'medium', emoji: '🟡', label: t.awk_urgency_medium || 'Umiarkowanie' },
...
Wygeneruj kompleksową radę jak poradzić sobie z tą niezręczną sytuacją.`
      ...
      setError(t.awk_error || 'Wystąpił błąd podczas generowania rady. Spróbuj ponownie.');
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  if (!showResult) {
    return (
      <LinearGradient colors={['#0f172a', '#2e1065', '#0f172a']} style={styles.gradient}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 24, paddingTop: 40, alignItems: 'center' }}>
          <TouchableOpacity onPress={() => onNavigate('dashboard')} style={{ padding: 4 }}>
            <ArrowLeft color="white" size={24} />
          </TouchableOpacity>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <View style={{ width: 32, height: 32, backgroundColor: '#fb7185', borderRadius: 8, alignItems: 'center', justifyContent: 'center' }}>
              <Heart color="white" size={18} />
            </View>
            <Text style={{ color: 'white', fontSize: 18, fontWeight: '500' }}>{t.dash_awk}</Text>
          </View>
          <View style={{ width: 40 }} />
        </View>

        <ScrollView contentContainerStyle={{ padding: 24, paddingTop: 0 }}>
          <View style={[styles.card, { backgroundColor: 'rgba(244, 63, 94, 0.1)', borderColor: 'rgba(244, 63, 94, 0.3)', marginBottom: 24 }]}>
            <Text style={{ color: '#fb7185', fontSize: 14, lineHeight: 20 }}>
              {t.awk_description || 'Opisz niezręczną sytuację, a wygenerujemy profesjonalną radę.'}
            </Text>
          </View>
            ...
            {categories.map(cat => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  borderRadius: 20,
                  backgroundColor: selectedCategory === cat.id ? '#fb7185' : 'rgba(30, 41, 59, 0.8)',
                  borderWidth: 1,
                  borderColor: selectedCategory === cat.id ? '#fb7185' : 'rgba(255,255,255,0.1)'
                }}
              >
                <Text style={{ fontSize: 20, marginBottom: 4 }}>{cat.emoji}</Text>
                <Text style={{ color: 'white', fontSize: 12 }}>{cat.label}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          ...
          <TextInput
            ...
            placeholder={t.awk_situation_placeholder || "Co się stało? Bądź szczery/a..."}
            placeholderTextColor="#475569"
            multiline
            maxLength={500}
            style={{
          ...
          />
          <Text style={{ color: '#64748b', textAlign: 'right', fontSize: 12, marginBottom: 24 }}>
            {situation.length}/500
          </Text>

          <Text style={{ color: 'white', fontSize: 16, fontWeight: '500', marginBottom: 12 }}>
            {t.awk_context || 'Dodatkowy kontekst'} <Text style={{ color: '#64748b', fontSize: 12 }}>(opcjonalnie)</Text>
          </Text>
          <TextInput
            value={context}
            onChangeText={setContext}
            placeholder={t.awk_context_placeholder || "Kim są osoby zaangażowane?"}
            placeholderTextColor="#475569"
            multiline
            maxLength={300}
            style={{
            ...
            {urgencyLevels.map(level => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  paddingVertical: 12,
                  borderRadius: 12,
                  backgroundColor: urgency === level.id ? 'rgba(244, 63, 94, 0.2)' : 'rgba(30, 41, 59, 0.5)',
                  borderWidth: 2,
                  borderColor: urgency === level.id ? '#fb7185' : 'rgba(255,255,255,0.1)',
                  alignItems: 'center'
                }}
              >
                <Text style={{ fontSize: 24, marginBottom: 4 }}>{level.emoji}</Text>
                <Text style={{ color: 'white', fontSize: 12 }}>{level.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <TouchableOpacity
            ...
            style={[
              styles.btnPink,
              {
                opacity: (!situation.trim() || !selectedCategory || isGenerating) ? 0.5 : 1,
                marginTop: 0,
                marginBottom: 40
              }
            ]}
  ...
  return (
    <LinearGradient colors={['#0f172a', '#2e1065', '#0f172a']} style={styles.gradient}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 24, paddingTop: 40, alignItems: 'center' }}>
        ...
        <Text style={{ color: 'white', fontSize: 18, fontWeight: '500' }}>{t.awk_your_advice || 'Twoja rada'}</Text>
      ...
      <ScrollView contentContainerStyle={{ padding: 24, paddingTop: 0 }}>
        <View style={[styles.card, { backgroundColor: 'rgba(244, 63, 94, 0.1)', borderColor: 'rgba(244, 63, 94, 0.3)', marginBottom: 20 }]}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <Text style={{ fontSize: 20 }}>{categories.find(c => c.id === selectedCategory)?.emoji}</Text>
          ...
          <Text style={{ color: '#cbd5e1', fontSize: 13, fontStyle: 'italic' }}>
            "{advice.situation.substring(0, 100)}{advice.situation.length > 100 ? '...' : ''}"
          </Text>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: 'rgba(34, 197, 94, 0.1)', borderColor: 'rgba(34, 197, 94, 0.3)', marginBottom: 20 }]}>
          <Text style={{ color: '#22c55e', fontSize: 16, fontWeight: '600', marginBottom: 16 }}>
        ...
        <View style={[styles.card, { backgroundColor: 'rgba(239, 68, 68, 0.1)', borderColor: 'rgba(239, 68, 68, 0.3)', marginBottom: 20 }]}>
          <Text style={{ color: '#ef4444', fontSize: 16, fontWeight: '600', marginBottom: 16 }}>
        ...
        <View style={[styles.card, { backgroundColor: 'rgba(168, 85, 247, 0.1)', borderColor: 'rgba(168, 85, 247, 0.3)', marginBottom: 40 }]}>
...
export const ReplySuggestionsScreen = ({ t, onNavigate }) => {
  ...
  const tones = [
    ...
    { id: 'professional', emoji: '💼', label: t.tone_professional || 'Profesjonalny' },
  ...
  const getToneDescription = (toneId) => {
    const descriptions = {
      ...
      balanced: 'zrównoważowany - ani zbyt formalny ani zbyt casualowy, uniwersalny',
...
FORMAT (zwróć TYLKO JSON):
{
    ...
    },
    {
      ...
      },
      {
        ...
        content: [
          {
            type: 'text',
            text: `Przeanalizuj ten zrzut ekranu konwersacji i wygeneruj 3 odpowiedzi w stylu: ${getToneDescription(selectedTone)}`
          },
          {
            type: 'image_url',
            image_url: {
    ...
    return (
      <LinearGradient colors={['#0f172a', '#2e1065', '#0f172a']} style={styles.gradient}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 24, paddingTop: 40, alignItems: 'center' }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <View style={{ width: 32, height: 32, backgroundColor: '#a855f7', borderRadius: 8, alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ fontSize: 18 }}>💬</Text>
            </View>
            <Text style={{ color: 'white', fontSize: 18, fontWeight: '500' }}>{t.dash_reply}</Text>
          </View>
          <View style={{ width: 40 }} />
        </View>

        <ScrollView contentContainerStyle={{ padding: 24, paddingTop: 0 }}>
          <View style={[styles.card, { backgroundColor: 'rgba(168, 85, 247, 0.1)', borderColor: 'rgba(168, 85, 247, 0.3)', marginBottom: 24 }]}>
            <Text style={{ color: '#a855f7', fontSize: 14, lineHeight: 20 }}>
              {t.reply_description || 'Wklej wiadomość lub dodaj zrzut ekranu, a AI wygeneruje 3 inteligentne odpowiedzi.'}
            </Text>
          </View>

          {error && (
            <View style={[styles.card, { backgroundColor: 'rgba(239, 68, 68, 0.1)', borderColor: 'rgba(239, 68, 68, 0.3)', marginBottom: 24 }]}>
              <Text style={{ color: '#ef4444', fontSize: 14 }}>❌ {error}</Text>
            </View>
          )}
          ...
          {!imageUri && (
            <>
              ...
              <TextInput
                ...
                placeholder={t.reply_placeholder || 'Wklej tutaj otrzymaną wiadomość...'}
                placeholderTextColor="#475569"
                multiline
                maxLength={600}
                style={{
            ...
            {tones.map(tone => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  paddingHorizontal: 20,
                  borderRadius: 20,
                  backgroundColor: selectedTone === tone.id ? '#a855f7' : 'rgba(30, 41, 59, 0.8)',
                  borderWidth: 1,
                  borderColor: selectedTone === tone.id ? '#a855f7' : 'rgba(255,255,255,0.1)'
                }}
              >
                <Text style={{ fontSize: 20, marginBottom: 4 }}>{tone.emoji}</Text>
                <Text style={{ color: 'white', fontSize: 12 }}>{tone.label}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          ...
          <TouchableOpacity
            ...
            style={[
              styles.btnGreen,
              {
                opacity: ((!messageText.trim() && !imageUri) || isGenerating) ? 0.5 : 1,
                marginTop: 0,
                marginBottom: 40
              }
            ]}
  ...
  return (
    <LinearGradient colors={['#0f172a', '#2e1065', '#0f172a']} style={styles.gradient}>
      ...
      <ScrollView contentContainerStyle={{ padding: 24, paddingTop: 0 }}>
        <View style={[styles.card, { backgroundColor: 'rgba(168, 85, 247, 0.1)', borderColor: 'rgba(168, 85, 247, 0.3)', marginBottom: 20 }]}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <Text style={{ fontSize: 18 }}>{tones.find(t => t.id === selectedTone)?.emoji}</Text>
            <Text style={{ color: '#a855f7', fontSize: 14, fontWeight: '600' }}>
              {suggestions.tone}
            </Text>
            {suggestions.fromImage && (
              <View style={{ backgroundColor: 'rgba(251, 191, 36, 0.2)', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8, marginLeft: 4 }}>
                <Text style={{ color: '#fbbf24', fontSize: 10 }}>📸 Z obrazu</Text>
              </View>
            )}
          </View>
          <Text style={{ color: '#cbd5e1', fontSize: 13, fontStyle: 'italic' }}>
            "{suggestions.originalMessage.substring(0, 80)}{suggestions.originalMessage.length > 80 ? '...' : ''}"
          </Text>
        </View>
          ...
          <View 
            ...
            style={[
              ...
              { 
                marginBottom: 16,
                borderWidth: 2,
                borderColor: index === 0 ? 'rgba(168, 85, 247, 0.5)' : 'rgba(255,255,255,0.1)'
              }
            ]}
          >
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
              <View style={{ 
                ...
                height: 32, 
                borderRadius: 16, 
                backgroundColor: index === 0 ? '#a855f7' : 'rgba(148, 163, 184, 0.2)',
                alignItems: 'center',
            ...
            <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 8, marginBottom: 12 }}>
              ...
              <Text style={{ color: '#94a3b8', fontSize: 13, lineHeight: 18, flex: 1 }}>
                {reply.why}
              </Text>
            </View>
        ...
        <View style={[styles.card, { backgroundColor: 'rgba(251, 191, 36, 0.1)', borderColor: 'rgba(251, 191, 36, 0.3)', marginBottom: 20 }]}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <Text style={{ fontSize: 18 }}>💡</Text>
            <Text style={{ color: '#fbbf24', fontSize: 16, fontWeight: '600' }}>
          ...
          <Text style={{ color: '#cbd5e1', fontSize: 13, lineHeight: 20 }}>
            {t.reply_tips_text || 'Te odpowiedzi zostały wygenerowane przez AI. Możesz je dopasować do siebie!'}
          </Text>
        </View>
        ...
        <View style={{ gap: 12, marginBottom: 40 }}>
          ...
          <TouchableOpacity 
            onPress={generateSuggestions}
            style={[styles.btnPrimary, { marginTop: 0, backgroundColor: 'rgba(168, 85, 247, 0.2)', borderWidth: 2, borderColor: '#a855f7' }]}
          >
            <RefreshCw color="#a855f7" size={18} />
            <Text style={{ color: '#a855f7', fontWeight: '600', fontSize: 16, marginLeft: 8 }}>
